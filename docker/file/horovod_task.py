...
class HorovodTask(ByteTask):
    ...
    def _partition_tensor(self, size, tensor_layer, delta_1, delta_2, O_lay, O_num):
        ...
        for i in range(delta_1+delta_2):
            if O_lay[i] == tensor_layer and O_num[i] != 0:
                number = number + 1
                fusion_num.append(int(O_num[i]))
                order.append(i)

        partitions = []
        start = 0
        end = num_per_partition * fusion_num[0]
        for i in range(number):
            if i == number-1:
                ...
                partitions.append(avatar)
                start = end
                end = min(end + num_per_partition * fusion_num[i+1], self._tensor.shape[0])
