...
class ByteTask(with_metaclass(ABCMeta)):
    ...
    def partition(self, size=None, delta_1=0, delta_2=0, O_lay=[0], O_num=[0]):
        ...
        tensor_layer = self.priority // 1000

        # Partition tasks evenly or based on a list
        if isinstance(size, list):
            tensor_partitions = self._partition_tensor_v2(size)
        else:
            tensor_partitions, order = self._partition_tensor(size, tensor_layer, delta_1, delta_2, O_lay, O_num)
    ...
    @abstractmethod
    def _partition_tensor(self, size, tensor_layer, delta_1, delta_2, O_lay, O_num):
        """Must be overriden -- required by Core.
